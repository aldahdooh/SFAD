...
from models.mnist.multi_task_adv_selective_model_mnist_v7 import multi_mnist_model as model_mnist_adv

class multi_mnist_model:
    def __init__(self, mode='train', no_defense_h5="mnist_model_1.h5", filename_a="multi_mnist_v7.h5", filename_b="multi_mnist_model_v7b.h5", coverage=0.95, coverage_th=0.5, alpha=0.25, normalize_mean=False):
        self.mode = mode
        ...
        if mode=='train':
            ...
            self.model.load_weights("checkpoints/mnist/{}".format(self.filename_b))
        else:
    ...
    def train(self, model):
        ...
        with open("checkpoints/mnist/{}_history.pkl".format(self.filename_b[:-3]), 'wb') as handle:
            pickle.dump(historytemp.history, handle, protocol=pickle.HIGHEST_PROTOCOL)

        model.save_weights("checkpoints/mnist/{}".format(self.filename_b))
