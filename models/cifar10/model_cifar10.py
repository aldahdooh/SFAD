...
class cifar10_model:
    def __init__(self, mode='train', filename="cifar10_model.h5", normalize_mean=False):
        self.mode = mode
        ...
        if mode=='train':
            ...
            self.model.load_weights("checkpoints/cifar10/{}".format(self.filename))
        else:
    ...
    def build_model(self):
        ...
        task0 = Conv2D(64, (3, 3), padding='same', kernel_regularizer=regularizers.l2(weight_decay), name='l_1')(input)
        task0 = BatchNormalization(name='l_2')(task0)
        task0 = Activation('relu', name='l_3')(task0)
        
        task0 = Conv2D(64, (3, 3), padding='same', kernel_regularizer=regularizers.l2(weight_decay), name='l_4')(task0)
        ...
        task0 = Conv2D(128, (3, 3), padding='same', kernel_regularizer=regularizers.l2(weight_decay), name='l_9')(task0)
        task0 = BatchNormalization(name='l_10')(task0)
        task0 = Activation('relu', name='l_11')(task0)

        task0 = Conv2D(128, (3, 3), padding='same', kernel_regularizer=regularizers.l2(weight_decay), name='l_12')(task0)
        ...
        task0 = Conv2D(256, (3, 3), padding='same', kernel_regularizer=regularizers.l2(weight_decay), name='l_17')(task0)
        task0 = BatchNormalization(name='l_18')(task0)
        task0 = Activation('relu', name='l_19')(task0)

        task0 = Conv2D(256, (3, 3), padding='same', kernel_regularizer=regularizers.l2(weight_decay), name='l_20')(task0)
        ...
        task0 = Conv2D(512, (3, 3), padding='same', kernel_regularizer=regularizers.l2(weight_decay), name='l_25')(task0)
        ...
        task0 = Dense(512, kernel_regularizer=regularizers.l2(weight_decay), name='l_31')(task0)
    ...
    def train(self, model):
        ...
        with open("checkpoints/cifar10/{}_history.pkl".format(self.filename[:-3]), 'wb') as handle:
            pickle.dump(historytemp.history, handle, protocol=pickle.HIGHEST_PROTOCOL)

        model.save_weights("checkpoints/cifar10/{}".format(self.filename))
