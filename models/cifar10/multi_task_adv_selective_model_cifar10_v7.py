...
class multi_cifar10_model:
    def __init__(self, mode='train', no_defense_h5="cifar10_model_1.h5", filename="multi_cifar10_model_v7.h5", coverage=0.95, coverage_th=0.5, alpha=0.5 , normalize_mean=False):
        self.mode = mode
        ...
        from models.cifar10.model_cifar10 import cifar10_model as no_defense_model
        ...
        ##layers data train
        inter_model = Model(inputs=no_defense_model.get_input_at(0), outputs=no_defense_model.get_layer('l_31').output)
        ...
        inter_model = Model(inputs=no_defense_model.get_input_at(0), outputs=no_defense_model.get_layer('l_26').output)
        ...
        inter_model = Model(inputs=no_defense_model.get_input_at(0), outputs=no_defense_model.get_layer('l_21').output)
        ...
        if mode=='train':
            ...
            self.model_1.load_weights("checkpoints/cifar10/{}".format(self.filename[:-3]+'_model_1.h5'))
            self.model_2.load_weights("checkpoints/cifar10/{}".format(self.filename[:-3]+'_model_2.h5'))
            self.model_3.load_weights("checkpoints/cifar10/{}".format(self.filename[:-3]+'_model_3.h5'))
        else:
    ...
    def train(self, model_1, model_2, model_3):
        ...
        with open("checkpoints/cifar10/{}_history_model_1.pkl".format(self.filename[:-3]), 'wb') as handle:
            pickle.dump(historytemp.history, handle, protocol=pickle.HIGHEST_PROTOCOL)

        model_1.save_weights("checkpoints/cifar10/{}_model_1.h5".format(self.filename[:-3]))
        ...
        with open("checkpoints/cifar10/{}_history_model_2.pkl".format(self.filename[:-3]), 'wb') as handle:
            pickle.dump(historytemp.history, handle, protocol=pickle.HIGHEST_PROTOCOL)

        model_2.save_weights("checkpoints/cifar10/{}_model_2.h5".format(self.filename[:-3]))
        ...
        with open("checkpoints/cifar10/{}_history_model_3.pkl".format(self.filename[:-3]), 'wb') as handle:
            pickle.dump(historytemp.history, handle, protocol=pickle.HIGHEST_PROTOCOL)

        model_3.save_weights("checkpoints/cifar10/{}_model_3.h5".format(self.filename[:-3]))
